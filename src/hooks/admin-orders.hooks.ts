...
export const adminOrdersKeys = {
  ...
  list: (params: GetOrdersParams) => [...adminOrdersKeys.lists(), params] as const,
...
export const useAddRider = () => {
  ...
  return useMutation({
    mutationFn: ({ orderId, data }: { orderId: string; data: AddRiderRequest }) =>
...
export const useUpdateRider = () => {
  ...
  return useMutation({
    mutationFn: ({ orderId, data }: { orderId: string; data: UpdateRiderRequest }) =>
    ...
    onError: (error: unknown) => {
      const axiosError = error as { response?: { data?: { message?: string } } };
      toast.error(axiosError.response?.data?.message || "Failed to update rider information");
    },
  });
};
...
export const useUpdateOrderStatus = () => {
  ...
  return useMutation({
    ...
    onError: (error: unknown) => {
      const axiosError = error as { response?: { data?: { message?: string } } };
      toast.error(axiosError.response?.data?.message || "Failed to update order status");
    },
  });
};
