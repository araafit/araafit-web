...
export function DashboardFabricRequestFlowPage() {
  ...
  const { data: product, isLoading } = useProduct(rawParam);
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#F5F5F5] flex items-center justify-center px-4 py-6 md:px-8">
        <div className="h-full flex flex-col items-center justify-center">
          <Spinner isLoading={isLoading} size="xl" speed="fast" arcColor="#9A6C50" className="mb-2" />
           <p className="text-sm text-neutral-500">Loading fabric details</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <section className="min-h-screen bg-[#F5F5F5] flex items-start justify-center px-4 py-6 md:px-8">
      <div className="h-full max-w-[72rem] bg-white rounded-md shadow-sm p-4 md:p-6 flex flex-col gap-6">
        {/* Header row */}
