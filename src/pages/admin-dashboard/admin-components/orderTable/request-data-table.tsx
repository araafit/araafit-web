...
// Action cell component to enable hooks usage
const ActionCell: React.FC<{ row: Row<z.infer<typeof schema>> }> = ({ row }) => {
  ...
  return (
    <Drawer>
      ...
      <DrawerContent className="w-[59.063rem] flex flex-col h-screen max-h-screen relative">
        <DrawerHeader className="flex items-center gap-2">
            ...
            return (
              <div
                className={`px-2 py-1 text-xs rounded-full w-fit ${
                  style ? `${style.bgColor} ${style.textColor}` : "bg-gray-100 text-gray-600"
                }`}
              >
                {status}
              </div>
          ...
          {row.original.status !== "Pending" && row.original.status !== "Canceled" && (
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-52 h-full flex flex-col items-start gap-3 p-3">
                {orderStatuses.map((status) => (
                  <DropdownMenuItem
                    className={`${status.bgColor} ${status.textColor} px-2 cursor-pointer text-xs py-1 w-auto block rounded-full ${isUpdating ? "pointer-events-none opacity-70" : ""}`}
                    key={status.status}
                    onClick={(e) => {
                      e.stopPropagation();
        ...
        {row.original.status !== "Complete" && row.original.status !== "Canceled" && (
                ...
                <Dialog>
                  <DialogTrigger asChild>
                    <Button
                      type="button"
                      text="Add Rider Info"
                      icon={<PlusIcon className="size-[1.25rem] text-[#3D3D3D]" />}
                      variant="outline"
